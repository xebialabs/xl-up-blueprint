            ...
            steps {
                script {
                    try {
                        ...
                        sh (script: "sed -e 's@https://aws-eks.com:6443@$eksEndpoint@g' xl-up/__test__/test-cases/external-db/eks-xld-xlr-mon.yaml")
                        sh (script: "sed -e 's@SOMEKEY@$awsAccessKey@g' xl-up/__test__/test-cases/external-db/eks-xld-xlr-mon.yaml")
